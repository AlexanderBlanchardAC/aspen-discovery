{strip}
	...
	{foreach from=$libraryLocations item=curLocation name=locationLoop}
		<div class="locationInfo" id="locationAddress{$curLocation.id}"
			...
			{/if}
				{if !empty($curLocation.latitude) && $curLocation.latitude !== 0 || (!empty($curLocation.useLocationNameForMaps) && $curLocation.useLocationNameForMaps == 1)}
				<div style="flex-grow: 1; flex-basis: 300px; flex-shrink: 0; justify-self: stretch">
					<iframe
							width="100%"
							height="250"
							...
							src="https://maps.google.com/?ie=UTF8&t=m{if !empty($curLocation.useLocationNameForMaps) && $curLocation.useLocationNameForMaps == 1}&q={$curLocation.address|escape:'url'}{else}&ll={$curLocation.latitude},{$curLocation.longitude}&spn=0.003381,0.017231{/if}&z=16&output=embed">
					</iframe>
				</div>
				{/if}
			</div>
