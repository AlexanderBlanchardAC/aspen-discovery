 ...
 */
function _processCampaignMilestone($value, $objectType, $userId, $date, $groupedId = null) {
...
add_action('after_object_insert', 'after_checkout_insert', function ($value) {
	_processCampaignMilestone($value, 'user_checkout', $value->userId, $value->checkoutDate, $value->groupedWorkId);
});
...
add_action('after_object_insert', 'after_hold_insert', function ($value) {
	_processCampaignMilestone($value, 'user_hold', $value->userId, $value->createDate, $value->groupedWorkId);
});
...
add_action('after_object_insert', 'after_work_review_insert', function ($value) {
	_processCampaignMilestone($value, 'user_work_review', $value->userId, $value->dateRated, $value->groupedRecordPermanentId);
});
