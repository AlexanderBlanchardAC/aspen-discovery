...
class SearchObject_SummonSearcher extends SearchObject_BaseSearcher {
    ...
    private $summonSearchSettings;
    /** @var SummonSetting */
    private $summonSettings;
    // protected $host = 'http://api.summon.serialssolutions.com';
    protected $version = '2.0.0';
    private $summonBaseUrl = 'http://api.summon.serialsolutions.com';
    ...
    public function __construct() {
        ...
        global $library;
        if ($library->summonSearchSettingId > 0) {
            $searchSettings = new SummonSearchSetting();
            $searchSettings->id = $library->summonSearchSettingId;
            if (!$searchSettings->find(true)) {
                $searchSettings = null;
            }
            $this->summonSearchSettings = $searchSettings;
        }
    }
    ...
    public function getSearchSettings(): ?SummonSearchSetting {
        return $this->summonSearchSettings;
    }

    /** @return SummonSetting|null */

    private function getSettings(): ?SummonSetting {
        global $library;
        if ($this->summonSettings == null) {
            require_once ROOT_DIR . '/sys/Summon/SummonSearchSetting.php';
            $searchSettings = new SummonSearchSetting();
            $searchSettings->id = $library->summonSearchSettingId;
            if ($searchSetttings->find(true)) {
                $this->summonSettings = new SummonSetting();
                $this->summonSettings->id = $searchSettings->settingId;
    ...
    public function setSettings($summonSettings) {
        $this->summonSettings = $summonSettings;
    }
    ...
    public function getSearchOptions(): ?SimpleXMLElement {
		if (SearchObject_SummonSearcher::$searchOptions == null) {
			...
			if ($settings != null) {
				...
				$infoUrl = $this->summonBaseUrl . "/Info?prof={$settings->profileId}&pwd={$settings->profilePwd}";
				curl_setopt($curlConnection, CURLOPT_URL, $infoUrl);
				$searchOptionsStr = curl_exec($curlConnection);

				SearchObject_SummonSearcher::$searchOptions = simplexml_load_string($searchOptionsStr);
    ...
    public function getSearchIndexes() {
		...
		if ($settings == null) {
			...
			$searchIndexes = $memCache->get('summon_search_indexes_' . $settings->profileId);
...
//				}
				global $configArray;
				$memCache->set('summon_search_indexes_' . $settings->profileId, $searchIndexes, $configArray['Caching']['summon_options']);
			}
	...
	public function getSortOptions() {
		...
		if (empty($appliedDatabases)) {
			...
			if (!empty($this->summonSearchSettings)) {
				$defaultDatabases = $this->summonSearchSettings->getDefaultSearchDatabases();
			} else {
				$defaultDatabases = [];
			}
	...
	public function getFacetSet() {
		...
		if ($searchOptions != null) {
			...
			if (!empty($this->summonSearchSettings)) {
				foreach ($this->summonSearchSettings->getDatabases() as $database) {
			...
			if (!empty($this->lastSearchResults->Facets)) {
				if (!empty($this->lastSearchResults->Facets->Clusters)) {
					foreach ($this->lastSearchResults->Facets->Clusters->ClusterCategory as $facetCluster) {
						...
						require_once ROOT_DIR . '/sys/Summon/SummonFacet.php';
						$summonFacet = new SummonFacet();
						$summonFacet->shortName = $tag;
						if (!$summonFacet->find(true)) {
							//Add this to the database so we can display the name later

							$displayName = preg_replace('~([a-z])([A-Z])~', '\\1 \\2', $id);
							$summonFacet->displayName = $displayName;
							$summonFacet->insert();
						} else {
							$displayName = $summonFacet->displayName;
						}
	...
	public function retrieveRecord($dbId, $uniqueIdTag, $uniqueId) {
		...
		if ($settings != null) {
			...
			//check this
			$infoUrl = $this->summonBaseUrl . "/Search?prof={$settings->profileId}&pwd={$settings->profilePwd}&format=full";
	...
	function getSearchesFile() {
		// Summon does not have a searches file, we load dynamically
		return false;
	}
	...
	public function processSearch($returnIndexErrors = false, $recommendations = false, $preventQueryModification = false) {
		$settings = $this->getSettings();
		if ($settings == null) {
			return new AspenError("Summon searching is not configured for this library.");
		} else {
			$this->startQueryTimer();
			$hasSearchTerm = false;
			$searchUrl = $this->summonBaseUrl . "/Search?authType=profile&prof={$this->getSettings()->profileId}&pwd={$this->getSettings()->profilePwd}&format=detailed";
			...
			if (!$hasAppliedDatabase) {
				...
				if ($this->summonSearchSettings != null) {
					$defaultSearchDatabases = $this->summonSearchSettings->getDefaultSearchDatabases();
			...
			try {
				...
				if ($searchData->Message) {
					...
					return new AspenError("Error processing search in Summon: " . $message);
				}
				$this->stopQueryTimer();
				if ($searchData && empty($searchData->ErrorNumber)) {
					...
					return new AspenError("Error processing search in Summon: " . (string)$searchData->ErrorNumber);
				} else {
					global $logger;
					...
					return new AspenError("Error processing search in Summon, unknown error returned.");
				}
			} catch (Exception $e) {
				global $logger;
	...
	public function getRecordDriverForResult($current): SummonRecordDriver {
		require_once ROOT_DIR . '/RecordDrivers/SummonRecordDriver.php';
		return new SummonRecordDriver($current);
	}
	...
	public function getFilterList() {
		...
		foreach ($this->filterList as $field => $values) {
			if ($field == 'db') {
				...
				require_once ROOT_DIR . '/sys/Summon/SummonFacet.php';
				$summonFacet = new SummonFacet();
				$summonFacet->shortName = $field;
				if ($summonFacet->find(true)) {
					$facetLabel = $summonFacet->displayName;
				} else {
					$facetLabel = $field;
				}
			}
