...
class Event extends DataObject {
	...
	private function clearFutureInstances() : void {
		$instance = new EventInstance();
		$instance->eventId = $this->id;
		$todayDate = date('Y-m-d');
		$todayTime = date('H:i:s');
	...
	public function setTypeField($fieldId, $value) : void {
	...
	public function setDatesPreview($value) : void {
		$this->_datesPreview = $value;
	}

	public function getTypeField($fieldId) : string {
	...
	public function getAllTypeFields() : array {
		$typeFields = [];
		$field = new EventEventField();
		$field->eventId = $this->id;
		$field->find();
	...
	public function getDatesPreview() : string {
	...
	public function getExistingDates() : string {
		if (!isset($this->_dates) && $this->id) {
			$this->_dates = [];
			$instance = new EventInstance();
			$instance->eventId = $this->id;
	...
	public function calculateEnd($fieldName) : ?string {
		$endDate = '';
		$endTime = '';
		if (isset($this->startDate) && isset($this->startTime) && isset($this->eventLength)) {
			try {
				...
				global $logger;
				$logger->log("Error calculating end time " . $ex->getMessage(), Logger::LOG_ERROR);
			}
		}
